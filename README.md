...
Metabase는 WSL Docker로 실행하며, 데이터 손실을 막으려면 반드시 **볼륨을 마운트**해야 합니다.
...
> 볼륨 없이 실행하면 컨테이너 재생성 시 대시보드·카드·사용자 설정이 전부 초기화됩니다.

### 볼륨 확인

```powershell
# 볼륨이 마운트되어 있는지 확인 — Mounts=[] 이면 위험
...
| 항목 | 로컬 현재 방식 | 원격 서버 권장 방식 |
|---|---|---|
| DB 저장소 | 컨테이너 내부 H2 (`metabase.db`) | PostgreSQL 전환 권장 (H2는 프로덕션 비권장) |
| 볼륨 | named volume `metabase-data` | 원격 디스크 또는 EFS 마운트 |
| 공개 URL | `http://localhost:3000` | HTTPS 도메인으로 교체 필요 |
...
| 마이그레이션 | 확인 테이블 | 확인 쿼리 예시 |
|---|---|---|
| `001_portal_schema.sql` | `works`, `channels` | `SELECT COUNT(*) FROM works;` |
| `011_naver_rights_holders_metabase_url.sql` | `naver_rights_holders` | `SELECT metabase_embed_url FROM naver_rights_holders LIMIT 3;` |
| `016_copyright_claim_documents.sql` | `copyright_claim_documents` | `SELECT COUNT(*) FROM copyright_claim_documents;` |
| `019_official_documents_work_scope.sql` | `official_documents` | `SELECT work_scope FROM official_documents LIMIT 1;` |

**3. integration_runs 테이블 확인 (대시보드 Runner 필수)**

```sql
SELECT COUNT(*) FROM integration_runs;
```

> 이 테이블이 없으면 대시보드 Runner가 DB 저장에 실패합니다. 해당 마이그레이션 파일을 적용했는지 확인하세요.
