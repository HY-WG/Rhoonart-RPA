...
import { collectNaverSupabaseReports, fetchNaverAnalyticsOptions, fetchNaverContentCatalog, fetchNaverRightsHolders } from "@/lib/api";

function numberText(value: number | undefined) {
  return Number(value || 0).toLocaleString("ko-KR");
}

export default function NaverSupabaseReportPage() {
  const queryClient = useQueryClient();
  const [maxClips, setMaxClips] = useState(2000);
  const [confirmOpen, setConfirmOpen] = useState(false);
  const contentQuery = useQuery({ queryKey: ["naver-content-catalog"], queryFn: fetchNaverContentCatalog });
  const rightsQuery = useQuery({ queryKey: ["naver-rights-holders"], queryFn: fetchNaverRightsHolders });
  const optionsQuery = useQuery({ queryKey: ["naver-analytics-options"], queryFn: fetchNaverAnalyticsOptions });
  const collectMutation = useMutation({
    mutationFn: collectNaverSupabaseReports,
    onSuccess: () => {
      setConfirmOpen(false);
      queryClient.invalidateQueries({ queryKey: ["naver-analytics-options"] });
    },
  });

  const enabledContents = useMemo(
    () => (contentQuery.data || []).filter((item) => item.naver_report_enabled || item.status === "Active" || item.active_flag === "Active"),
    [contentQuery.data],
  );
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex flex-col gap-4 border-b border-slate-200 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-semibold text-blue-700">{"NAVER CLIP"}</p>
          <h1 className="mt-1 text-2xl font-bold text-slate-950">{"Naver Supabase \uc5c5\ub370\uc774\ud2b8"}</h1>
          <p className="mt-2 max-w-3xl text-sm text-slate-500">
            {"Supabase\uc5d0 \ub4f1\ub85d\ub41c Naver \uc791\ud488/\uad8c\ub9ac\uc0ac \ub300\uc0c1\uc744 \uae30\uc900\uc73c\ub85c \ub124\uc774\ubc84 \ud074\ub9bd \uc131\uacfc\ub97c \uc218\uc9d1\ud558\uace0 daily/history \ud14c\uc774\ube14\uc744 \uc5c5\ub370\uc774\ud2b8\ud569\ub2c8\ub2e4."}
          </p>
        </div>
        <button
          type="button"
      ...
      {loading && <div className="rounded-lg border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">{"Naver \uc124\uc815\uc744 \ubd88\ub7ec\uc624\ub294 \uc911\uc785\ub2c8\ub2e4."}</div>}
              ...
              <div className="divide-y divide-slate-100">
                ...
                {enabledContents.slice(0, 10).map((item) => (
                  <div key={`${item.content_name}-${item.identifier}`} className="grid grid-cols-[1fr_120px_160px] gap-4 p-4 text-sm">
                    ...
                    <p className="text-right text-slate-500">{item.naver_report_enabled ? "enabled" : item.status || item.active_flag || "-"}</p>
                  </div>
                ))}
              </div>
            </section>
